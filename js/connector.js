function spotlightSpotter(id) {
	d3.selectAll('.group-bar:not(#' + _.kebabCase(id) + '):not(.selected)').classed('shadow', true);
	d3.selectAll('.tick text:not(#' + _.kebabCase(id) + '):not(.selected)').classed('shadow', true);
	d3.selectAll('.dot:not(.' + _.kebabCase(id) + ')').classed('shadow', true);

	d3.select('.group-bar#' + _.kebabCase(id)).classed('spotlight', true);
	d3.select('.tick text#' + _.kebabCase(id)).classed('spotlight', true);
